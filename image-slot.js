...
(() => {
  ...
  const PUBLIC_STATE_FILE = 'image-slots.state.json';
  ...
  function load() {
    ...
    loadP = fetch(PUBLIC_STATE_FILE)
      .then((r) => r.ok ? r.json() : fetch(STATE_FILE).then((fallback) => fallback.ok ? fallback.json() : null))
      .then((j) => {
